...
const contracts = [
  {
    ...
    details:
      "You meet an old friend who needs help with a bounty. Complete the contract to earn a hefty reward.",
    goal: 1000,
    reward: 2000,
  },
...
const state = {
  ...
  cooldowns: {
    action: {
      baseMs: BASE_HUNT_COOLDOWN_MS,
      ms: BASE_HUNT_COOLDOWN_MS,
      readyAt: 0,
    },
    hire: {
      baseMs: BASE_HUNT_COOLDOWN_MS,
...
function getButtonValue(typeId) {
  const t = (state.buttonTypes && state.buttonTypes[typeId]) || {
    base: 1,
    multiplier: 1,
  };
...
function updateCooldownFromUpgrades() {
  ...
  const linearFactor =
    count >= REDUCE_CD_MAX_COUNT ? 0 : Math.max(0, 1 - 0.05 * count);
  ...
  for (const key of Object.keys(state.cooldowns)) {
    ...
    const newMs =
      linearFactor === 0 ? 0 : Math.max(100, Math.round(base * linearFactor));
    cd.ms = newMs;
...
function canHuntClick() {
  return canClick("action");
}
function startHuntCooldown() {
  return startCooldown("action");
}
...
function updateUI() {
  ...
  if (contractsBox) {
    ...
    if (
      !state.flags.contractsUnlocked &&
      crewTypes.some((c) => c.count >= 10)
    ) {
...
function load() {
  ...
  try {
    ...
    if (
      state.upgrades &&
      state.upgrades.reduceCooldownPurchased &&
      state.upgrades.reduceCooldown == null
    ) {
...
function updateLastSaved() {
  ...
  el.textContent =
    secs === 0 ? "Last saved just now" : `Last saved ${secs} seconds ago`;
}
...
if (autosaveToggleBtn) {
  autosaveToggleBtn.addEventListener("click", () => {
    ...
    autosaveToggleBtn.textContent = autosaveEnabled
      ? "Autosave ON"
      : "Autosave OFF";
